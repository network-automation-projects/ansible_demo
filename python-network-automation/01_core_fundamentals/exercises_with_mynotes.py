...
def assign_sequential_ips(devices: List[str], base_ip: str) -> Dict[str, str]:
    ...
    for index, device_name in enumerate(devices, start=1):
        result[device_name] = f"{base_ip}{index}"
...
def round_robin_device(devices: List[str], index: int) -> str:
    ...
    return devices[index % len(devices)]  # replace with modulo expression
...
def should_throttle(operation_index: int, every_n: int) -> bool:
    ...
    return operation_index % every_n == 0  # replace with expression to pause every Nth operation.
...
def device_belongs_to_worker(
    ...
    return device_id % num_workers == worker_id  # replace with modulo expression

    # let's say there are 4 workers
    # 1 % 4 = 1 so that would belong to worker 1
    # 4 % 4 = 0 so that would belong to worker 0
    # 5 % 4 = 1 so that would belong to worker 1
    # 6 % 4 = 2 so that would belong to worker 2

    # 1 % 1 = 0 so that would belong to worker 0  (different scenario with 1 worker)
...
def backoff_delay_seconds(delays: List[int], attempt: int) -> int:
    ...
    return delays[attempt % len(delays)]  # replace with delays[attempt % len(delays)]

    # 1st attempt would be 0 % 4 = 0
    # 6th attempt would be 5 % 4 = 1 ?
...
def is_start_of_batch(index: int, batch_size: int) -> bool:
    ...
    return index % batch_size == 0      # replace with modulo expression
...
def stable_controller_for_device(
    ...
    return controllers[hash(device_name) % len(controllers)]  # replace with controllers[hash(device_name) % len(controllers)]
