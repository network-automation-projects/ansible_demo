...
class DeviceWithSummary(Device):
    ...
    def summary(self) -> str:
          # return a string using self.hostname, self.ip, self.vendor
...
class DeviceInventory:
    ...
    def __init__(self) -> None:
        self._devices: Dict [str, Device] = {}  # replace
...
if __name__ == "__main__":
    ...
    # Router (subclass of Device) — uncomment to run
    r = Router("router1", "10.0.0.1", "cisco", "ASR1000", "15.1")
    assert r.hostname == "router1"
    assert r.ip == "10.0.0.1"
    assert r.vendor == "cisco"
    assert r.model == "ASR1000"
    assert r.version == "15.1"
    assert isinstance(r, Device)
    print("Router tests passed:", r.hostname, r.model, r.version)
