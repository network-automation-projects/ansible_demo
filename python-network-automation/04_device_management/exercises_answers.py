...
def connect_to_device(device_info: Dict[str, str]) -> Any:
    ...
    return ConnectHandler(**device_info)    # connectionhandler expects an unpacked dictionary?
...
def configure_interface(
    ...
    configure_interface automates configuring a Cisco IOS interface:
    Enter config mode and select the interface (interface GigabitEthernet0/1, etc.).
    Set the IP address and subnet mask (ip address 10.0.0.1 255.255.255.0).
    Bring the interface up (no shutdown).
    Instead of typing those commands manually, this function sends them through the Netmiko connection.
    """
    config_commands = [
