...
class IngestRequest (BaseModel):
    ...
    #overwrite_if_exists: bool = True               # if True, allow re-upload for the same doc_id (replace existing); if False, keep current behavior and reject with 409 when doc_id exists.
...
class AskRequest(BaseModel):
    ...
    doc_id: str | None = Field(default=None, description="doc_id to search only within that document") #(if provided, search only within that document)
    use_rag: bool = True
